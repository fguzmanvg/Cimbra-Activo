...
function getStreak(logs) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    const diff = Math.round((current - new Date(sorted[i])) / 86400000);
    if (diff <= 1) { streak++; current = new Date(sorted[i]); }
    else break;
  }
  return streak;
}
...
export default function App() {
  ...
  useEffect(() => {
    const dataRef = ref(db, "activities");
  ...
  function getMemberData(id) {
    return data[id] || { logs: [], notes: [], progress: 0 };
  }
  ...
  async function submitActivity(memberId) {
    ...
    const md = getMemberData(memberId);
    const logs = [...(md.logs || []), today];
    const notes = [...(md.notes || []), {
      date: today,
      text: logActivity.trim() + (logNote.trim() ? " — " + logNote.trim() : ""),
      ts: Date.now()
    }];
  ...
  const activeMembers = MEMBERS.filter(m => (data[m.id]?.logs?.length || 0) > 0).length;
  ...
  return (
    <div style={{ background: "#0A0A0F", minHeight: "100vh", fontFamily: "'Georgia', serif", color: "#F0EDE8", position: "relative" }}>
      <div style={{
        position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0,
        background: "radial-gradient(ellipse at 20% 20%, rgba(255,107,53,0.06) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(78,205,196,0.05) 0%, transparent 50%)"
      }} />
      ...
      <header style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button onClick={() => setUserPicker(true)} style={{
            background: currentUser ? "rgba(255,107,53,0.15)" : "rgba(255,255,255,0.06)",
            border: `1px solid ${currentUser ? "rgba(255,107,53,0.4)" : "rgba(255,255,255,0.12)"}`,
            borderRadius: 20, padding: "6px 12px", color: currentUser ? "#FF6B35" : "#aaa",
          ...
          <div style={{ display: "flex", background: "rgba(255,255,255,0.04)", borderRadius: 8, border: "1px solid rgba(255,255,255,0.08)" }}>
            {[["board", "📊"], ["log", "✍️"]].map(([v, icon]) => (
              <button key={v} onClick={() => setView(v)} style={{
                padding: "7px 14px", background: view === v ? "rgba(255,107,53,0.2)" : "transparent",
                border: "none", color: view === v ? "#FF6B35" : "#666",
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {MEMBERS.map(m => (
                <button key={m.id} onClick={() => { saveUser(m.id); setUserPicker(false); setView("log"); }} style={{
                  background: currentUser === m.id ? "rgba(255,107,53,0.2)" : "rgba(255,255,255,0.04)",
                  border: `1px solid ${currentUser === m.id ? "rgba(255,107,53,0.5)" : "rgba(255,255,255,0.1)"}`,
                  borderRadius: 20, padding: "7px 14px",
                  color: currentUser === m.id ? "#FF6B35" : "#ccc",
      ...
      <div style={{
        ...
        {[
          ...
          [activeMembers, "han registrado"],
          [getDayOfYear() + "/365", "día del año"],
        ].map(([val, label]) => (
          ...
          <>
            <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto", paddingBottom: 4 }}>
              {[["all", "Todos 🌟"], ...Object.entries(CATEGORY_LABELS).map(([k, v]) => [k, v])].map(([cat, label]) => (
                <button key={cat} onClick={() => setFilterCat(cat)} style={{
                  flex: "0 0 auto",
                  background: filterCat === cat ? (cat === "all" ? "rgba(255,107,53,0.2)" : `${CATEGORY_COLORS[cat]}20`) : "rgba(255,255,255,0.04)",
                  border: `1px solid ${filterCat === cat ? (cat === "all" ? "#FF6B35" : CATEGORY_COLORS[cat]) : "rgba(255,255,255,0.1)"}`,
                  borderRadius: 20, padding: "5px 12px",
                  color: filterCat === cat ? (cat === "all" ? "#FF6B35" : CATEGORY_COLORS[cat]) : "#666",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 12 }}>
              {filtered.map((member, i) => {
                const md = getMemberData(member.id);
                const streak = getStreak(md.logs || []);
                const progress = md.progress || 0;
                const logCount = md.logs?.length || 0;
                ...
                return (
                  <div key={member.id}
                    ...
                    style={{
                      background: isMe ? "rgba(255,107,53,0.07)" : "rgba(255,255,255,0.025)",
                      border: `1px solid ${isMe ? "rgba(255,107,53,0.3)" : "rgba(255,255,255,0.07)"}`,
                      borderRadius: 14, padding: 16, cursor: "pointer",
                      animation: `fadeIn 0.4s ease ${i * 0.02}s both`
                    }}
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 9, letterSpacing: 1.5, textTransform: "uppercase", color, marginTop: 1 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#777", lineHeight: 1.5 }}>
                      {member.challenges[0]}
                      {member.challenges.length > 1 && <span style={{ color: "#444" }}> +{member.challenges.length - 1}</span>}
                    </div>

                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontSize: 10, color: "#444" }}>
                      <span>{logCount} actividades</span>
                    ...
                    {lastNote && (
                      <div style={{
                        ...
                        fontSize: 11, color: "#555", borderLeft: `2px solid ${color}44`,
                        fontStyle: "italic"
                      }}>
                        "{lastNote.text.length > 55 ? lastNote.text.slice(0, 55) + "…" : lastNote.text}"
                      </div>
                    )}
                  </div>
                );
          ...
          return (
            <div>
              <button onClick={() => setSelectedMember(null)} style={{
                ...
                fontSize: 13, marginBottom: 20, padding: 0, fontFamily: "Georgia,serif"
              }}>← tablero</button>
              ...
              {isMe && (
                <div style={{ background: "rgba(255,107,53,0.06)", border: "1px solid rgba(255,107,53,0.2)", borderRadius: 16, padding: 20, marginBottom: 20 }}>
                  ...
                  <input value={logActivity} onChange={e => setLogActivity(e.target.value)}
                    placeholder="¿Qué hiciste hoy?"
                    style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "12px 14px", color: "#F0EDE8", fontSize: 14, fontFamily: "Georgia,serif", marginBottom: 10, boxSizing: "border-box" }}
                  />
                  <input value={logNote} onChange={e => setLogNote(e.target.value)}
                    placeholder="Detalle: km, kg, páginas, tiempo..."
                    style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "10px 14px", color: "#999", fontSize: 13, fontFamily: "Georgia,serif", marginBottom: 14, boxSizing: "border-box" }}
                  />
                  <button onClick={() => submitActivity(member.id)} disabled={saving} style={{
                    width: "100%", background: saving ? "#333" : "#FF6B35", border: "none",
                    borderRadius: 10, padding: "13px", color: "#fff", fontSize: 15, fontWeight: "bold",
                    cursor: saving ? "default" : "pointer", fontFamily: "Georgia,serif"
          ...
          <div>
            {!currentUser ? (
              <div style={{ textAlign: "center", padding: "40px 20px" }}>
                ...
                <div style={{ color: "#666", marginBottom: 24, lineHeight: 1.6 }}>Primero dinos quién eres para poder registrar tu progreso</div>
                <button onClick={() => setUserPicker(true)} style={{
                  background: "rgba(255,107,53,0.2)", border: "1px solid #FF6B35",
                  borderRadius: 12, padding: "14px 28px", color: "#FF6B35",
                  cursor: "pointer", fontSize: 15, fontFamily: "Georgia,serif"
                }}>Seleccionar mi perfil</button>
              </div>
            ) : (() => {
              const member = MEMBERS.find(m => m.id === currentUser);
              const md = getMemberData(currentUser);
              const loggedToday = (md.logs || []).includes(getDateKey());
              const streak = getStreak(md.logs || []);
              const color = CATEGORY_COLORS[member.category];
                ...
                <div>
                  <div style={{ textAlign: "center", marginBottom: 24 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#555", marginTop: 6 }}>
                      🔥 {streak} días de racha &nbsp;·&nbsp; {md.logs?.length || 0} actividades totales
                    </div>
                    {loggedToday && (
                      <div style={{ marginTop: 8, fontSize: 12, color: "#4ECDC4" }}>✓ Ya registraste hoy</div>
                    )}
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: 11, letterSpacing: 2, color: "#FF6B35", marginBottom: 14, textTransform: "uppercase" }}>
                      ¿Qué hiciste hoy?
                    </div>
                    <input value={logActivity} onChange={e => setLogActivity(e.target.value)}
                      placeholder="Fui al gym, corrí 5km, leí 30 págs..."
                      style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "14px", color: "#F0EDE8", fontSize: 15, fontFamily: "Georgia,serif", marginBottom: 10, boxSizing: "border-box" }}
                    />
                    <input value={logNote} onChange={e => setLogNote(e.target.value)}
                      placeholder="Detalle: tiempo, distancia, páginas..."
                      style={{ width: "100%", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "12px", color: "#888", fontSize: 13, fontFamily: "Georgia,serif", marginBottom: 16, boxSizing: "border-box" }}
                    />
                    <button onClick={() => submitActivity(currentUser)} disabled={saving} style={{
                      width: "100%", background: saving ? "#222" : "linear-gradient(135deg, #FF6B35, #ff8f5e)",
                      border: "none", borderRadius: 12, padding: "16px", color: "#fff",
                      fontSize: 16, fontWeight: "bold", cursor: saving ? "default" : "pointer", fontFamily: "Georgia,serif"
                    }}>{saving ? "Guardando..." : "Registrar actividad ✓"}</button>
